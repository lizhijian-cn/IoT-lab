package com.topelec.control.rfid;
...
public class RfidControl {
    ...
    private static RfidControl mInstance;
    private final ConditionVariable cv = new ConditionVariable(false);
    private final ConditionVariable lfPasswordCv = new ConditionVariable(false);
    String rfidType;// "LF";"HF";"UHF"
    private RfidCommand mRfidCommand;
    private File device;
...
//    }
    ...
    public RfidControl(Handler updateHandler) {
...
//            mHFReadThread.finishRunning();
    ...
    private void initUart(String RfidType, String Password) throws IOException {
        ...
        if (RfidType.equals("LF")) {
            ...
            if (baudrate == RfidCommand.lfBautrate) {
                return;
            } else {
                HardwareControl.CloseSerialPort();
                isUartOpened = false;
                lfPassword = Password;
                rfidPort = RfidCommand.lfRfidPort;
                baudrate = RfidCommand.lfBautrate;
            }

        } else if (RfidType.equals("HF")) {

            if (baudrate == RfidCommand.hfBautrate) {
                return;
            } else {
                HardwareControl.CloseSerialPort();
                isUartOpened = false;
                rfidPort = RfidCommand.hfRfidPort;
                baudrate = RfidCommand.hfBautrate;
            }

        } else if (RfidType.equals("UHF")) {
            if (baudrate == RfidCommand.uhfBautrate) {
                return;
            } else {
                HardwareControl.CloseSerialPort();
                isUartOpened = false;
                rfidPort = RfidCommand.uhfRfidPort;
                baudrate = RfidCommand.uhfBautrate;
            }
...
//        isReturnedValue = false;
     ...
     * 激活卡片（寻卡）
     *
     * @param activeAll true寻天线范围内的所有卡
     *                  false寻未进入睡眠状态的卡
     */
    ...
    private void LF_Setup_Password_Mode(byte[] password) {
        ...
        int length = RfidCommand.set_password_mode.length + password.length;
        byte[] cmd = new byte[length];

        System.arraycopy(RfidCommand.set_password_mode.length, 0, cmd, 0, RfidCommand.set_password_mode.length);
        System.arraycopy(password, 0, cmd, RfidCommand.set_password_mode.length, password.length);
    ...
    private void LF_Read_Page_One() {
        try {
            mOutputStream.write(RfidCommand.read_page_one);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void UHF_Read_ID() {
        try {
            mOutputStream.write(RfidCommand.read_id_once);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
        ...
        private void verifyPassword() {
            ...
            while (counter-- > 0) {
                ...
                if (lfPasswordCv.block(500)) {
                    ...
                    msg.what = RfidCommand.LF_PASSWORD;
        ...
        public void initHFRfid() {
            ...
            while (counter-- > 0) {
                ...
                if (cv.block(500)) {   //设置类型；true if the condition was opened
                    //TODO：成功设置1）重置计数器；2）执行打开射频动作

                    while (counter-- > 0) {
                        m104bpc_FrequencyCtrl(true);
                        if (cv.block(500)) { //打开天线
                            //TODO:成功打开天线1）重置计数器；2）继续执行
                            //suspend = false;
                            break;
                        } else {
                            //TODO:打开不成功1）停止线程；2）抛错误信息到UI
                            if (counter == 0) {
                                msg.what = RfidCommand.HF_FREQ;
                    ...
                    //TODO：设置不成功1）停止线程；2）抛错误信息到UI
                    if (counter == 0) {
                        msg.what = RfidCommand.HF_TYPE;
                        data.putBoolean("result", false);
                        msg.setData(data);
                        uiHandler.sendMessage(msg);
                        Thread.currentThread().interrupt();//终端线程
...
//                        data.putBoolean("result",false);
                ...
                case 0x46:       //激活卡片，寻卡，成功
                    msg.what = RfidCommand.HF_ACTIVE;
                    if (g_ReceiveData[0] == 0x00) {
                        //TODO:成功激活卡片
                        data.putBoolean("result", true);
                    } else {
