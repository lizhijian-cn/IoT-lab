package com.topelec.control.zigbee;
...
public class SensorControl {
    ...
    private final LinkedList<byte[]> sendingQueue = new LinkedList<byte[]>();
    /**
     * 存放观察者
     */
    private final List<LedListener> ledListeners = new ArrayList<LedListener>();
    private final List<TempHumListener> tempHumListeners = new ArrayList<TempHumListener>();
    private final List<MotorListener> motorListeners = new ArrayList<MotorListener>();
    private final List<LightSensorListener> lightSensorListeners = new ArrayList<LightSensorListener>();
    private final List<InfraRedSensorListener> infraRedSensorListeners = new ArrayList<InfraRedSensorListener>();
    private final List<HallSensorListener> hallSensorListeners = new ArrayList<HallSensorListener>();
    private final List<SmokeSensorListener> smokeSensorListeners = new ArrayList<SmokeSensorListener>();
    private final List<DimmableLedListener> dimmableLedListeners = new ArrayList<DimmableLedListener>();
    private final List<ShakeSensorListener> shakeSensorListeners = new ArrayList<ShakeSensorListener>();
    private final List<AccelerationSensorListener> accelerationSensorListeners = new ArrayList<AccelerationSensorListener>();
    private final List<DopplerSensorListener> dopplerSensorListeners = new ArrayList<DopplerSensorListener>();
    private final List<PESensorListener> peSensorListeners = new ArrayList<PESensorListener>();
    private ZigbeeCommand mCommand;
    private File device;
...
//        mSendThread.start();
    ...
    private void initUart() throws IOException {
        ...
        device = new File(ZigbeeCommand.zigbeePort);
...
//                throw new SecurityException();
        ...
        mFd = HardwareControl.OpenSerialPort(device.getAbsolutePath(), ZigbeeCommand.bautrate, 0);
        if (mFd == null) {
...
//                mReadThread.interrupt();
    ...
    public void led1_On(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led1_on, isWaiting);
    }
    ...
    public void led1_Off(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led1_off, isWaiting);
    }
    ...
    public void led2_On(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led2_on, isWaiting);
    }
    ...
    public void led2_Off(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led2_off, isWaiting);
    }
    ...
    public void led3_On(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led3_on, isWaiting);
    }
    ...
    public void led3_Off(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led3_off, isWaiting);
    }
    ...
    public void led4_On(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led4_on, isWaiting);
    }
    ...
    public void led4_Off(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led4_off, isWaiting);
    }
    ...
    public void allLeds_On(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led_all_on, isWaiting);
    }
    ...
    public void allLeds_Off(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.led_all_off, isWaiting);
    }
    ...
    public void fanForward(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.fan_forward, isWaiting);
    }
    ...
    public void fanBackward(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.fan_backward, isWaiting);
    }
    ...
    public void fanStop(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.fan_stop, isWaiting);
    }
    ...
    public void checkTemperature(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_temp, isWaiting);
    }
    ...
    public void checkHumidity(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_hum, isWaiting);
    }
    ...
    public void checkBrightness(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_brightness, isWaiting);
    }
    ...
    public void checkSmoke(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_smoke, isWaiting);
    }
    ...
    public void checkHall(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_hall, isWaiting);
    }
    ...
    public void checkIR(boolean isWaiting) {
        ...
        queueToSend(ZigbeeCommand.get_IR, isWaiting);
    }
    ...
    public void steplessLedControl(byte progress, boolean isWaiting) {
        ...
        ZigbeeCommand.set_stepless_led[5] = progress;
        queueToSend(ZigbeeCommand.set_stepless_led, isWaiting);
    }
    ...
    public void checkShake(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_shake, isWaiting);
    }
    ...
    public void checkX(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_x, isWaiting);
    }

    public void checkY(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_y, isWaiting);
    }

    public void checkZ(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_z, isWaiting);
    }
    ...
    public void checkDoppler(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_doppler, isWaiting);
    }
    ...
    public void checkPE(boolean isWaiting) {
        queueToSend(ZigbeeCommand.get_PE, isWaiting);
    }
...
//        int temperature,humidity;
        ...
        switch (cmd[1]) {
            case ZigbeeCommand.LED_SENSOR://LED灯控制节点，暂无返回值
                notifyLedListener(cmd[2], cmd[3]);
                break;
            case ZigbeeCommand.FAN_SENSOR://电机控制节点，暂无返回值
                notifyMotorListener(cmd[3]);
                break;
            case ZigbeeCommand.TEMPHUM_SENSOR://温湿度控制节点，返回温度或者湿度信息
                temphum_value = (cmd[3] & 0xFF) * 256 + (cmd[4] & 0xFF);
                if (temphum_value != 0xFFFF)
                    notifyTempHumListener(cmd[2], temphum_value);
                break;
            case ZigbeeCommand.BRIGHTNESS_SENSOR://光照强度采集节点，返回光照强度是否超过阈值
                notifyLightSenserListener(cmd[3]);
                break;
            case ZigbeeCommand.IR_SENSOR://红外信号采集节点，返回红外信号是否被阻断
                notifyInfraRedSensorListener(cmd[3]);
                break;
            case ZigbeeCommand.HALL_SENSOR://霍尔磁场检测节点，返回是否检测到磁场
                notifyHallSensorListener(cmd[3]);
                break;
            case ZigbeeCommand.SMOKE_SENSOR://有害气体检测节点，返回检测到的有害气体浓度是否超过阈值
                notifySmokeSensorListener(cmd[3]);
                break;
            case ZigbeeCommand.STEPLESS_LED://无极调光灯控制节点，暂无返回值
                notifyDimmableLedListener(cmd[3]);
                break;
            case ZigbeeCommand.SHAKE_SENSOR://震动
                notifyShakeSensorListener(cmd[3]);
                break;
            case ZigbeeCommand.AD_SENSOR://三轴加速度
                ad_value = (cmd[3] & 0xFF) * 256 + (cmd[4] & 0xFF);
                notifyAccelerationSensorListener(cmd[2], ad_value);
                break;
            case ZigbeeCommand.DOPPLER_SENSOR://多普勒
                notifyDopplerSensorListener(cmd[3]);
                break;
            case ZigbeeCommand.PE_SENSOR://光电
    ...
    public interface LedListener {
        void LedControlResult(byte led_id, byte led_status);
    }
    ...
    public interface MotorListener {
        void motorControlResult(byte motor_status);
    }
    ...
    public interface TempHumListener {
        void tempHumReceive(byte senser_id, int senser_data);
    }
    ...
    public interface LightSensorListener {
        void lightSensorReceive(byte sensor_status);
    }

    public interface InfraRedSensorListener {
        void infraRedSensorReceive(byte sensor_status);
    }

    public interface HallSensorListener {
        void hallSensorReceive(byte sensor_status);
    }

    public interface SmokeSensorListener {
        void smokeSensorReceive(byte sensor_status);
    }

    public interface DimmableLedListener {
        void dimmableLedReceive(byte sensor_status);
    }

    public interface ShakeSensorListener {
        void shakeSensorReceive(byte sensor_status);
    }

    public interface AccelerationSensorListener {
        void accelerationSensorReceive(byte sensor_id, int sensor_data);
    }
    ...
    public interface DopplerSensorListener {
        void dopplerSensorReceive(byte sensor_status);
    }

    public interface PESensorListener {
        void peSensorReceive(byte sensor_status);
    }
...
//            while (isOver == false) {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    Thread.currentThread().sleep(ZigbeeCommand.CHECK_SENSOR_DELAY);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                    Thread.currentThread().interrupt();
                }
